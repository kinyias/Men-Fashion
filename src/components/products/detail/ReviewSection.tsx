...
export function ReviewsSection({ productId, averageRating, totalReviews }: ReviewsSectionProps) {
  ...
  const filteredAndSortedReviews = reviews
    ...
    .sort((a, b) => {
      switch (sortBy) {
        case "newest":
          return new Date(b.ngaydang).getTime() - new Date(a.ngaydang).getTime()
        case "oldest":
          return new Date(a.ngaydang).getTime() - new Date(b.ngaydang).getTime()
        case "highest":
  ...
  const renderStars = (rating: number, size: "sm" | "md" | "lg" = "md") => {
    ...
    return (
      <div className="flex">
        {[1, 2, 3, 4, 5].map((star) => {
          if (star <= rating) {
            return <Star key={star} className={`${sizeClasses[size]} fill-amber-400 text-amber-400`} />
          } else if (star - 0.5 <= rating) {
            return <StarHalf key={star} className={`${sizeClasses[size]} fill-amber-400 text-amber-400`} />
          } else {
  ...
  return (
    <div className="space-y-8">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">Dựa trên {totalReviews} đánh giá</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-3 bg-muted rounded-lg">
                  <div className="text-2xl font-bold text-green-600">
                    {totalReviews > 0 
                      ? Math.round((reviews.filter(r => r.sosao >= 4).length / totalReviews) * 100) 
                      : 0}%
                  </div>
                ...
                <div className="text-center p-3 bg-muted rounded-lg">
                  ...
                  <div className="text-sm text-muted-foreground">Số lượt đánh giá</div>
                </div>
              </div>
            </div>
          </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-8 text-red-500">Không thể tải đánh giá. Vui lòng thử lại sau.</div>
      ) : displayedReviews.length === 0 ? (
        <div className="text-center py-8">Chưa có đánh giá nào cho sản phẩm này</div>
      ) : (
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-medium">{review.nguoiDung.ho} {review.nguoiDung.ten}</span>
                        </div>
                          <p className="font-small text-gray-400">{review.nguoiDung.email}</p>
                        <div className="flex items-center gap-2 mt-1">
                          {renderStars(review.sosao, "sm")}
                          <span className="text-sm text-muted-foreground">{formatDate(review.ngaydang)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      {filteredAndSortedReviews.length > 3 && (
        <div className="text-center">
          ...
          >
            {showAllReviews ? "Hiển thị ít hơn" : `Xem tất cả ${filteredAndSortedReviews.length} đánh giá`}
