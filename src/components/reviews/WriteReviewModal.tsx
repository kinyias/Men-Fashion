...
import { checkRatingWithGemini } from '@/lib/gemini';
...
export function WriteReviewModal({
  ...
  const handleSubmit = async (data: DanhGiaFormValues) => {
    if (!user) {
    ...
    setIsLoading(true);
    const result = await checkRatingWithGemini(reviewData.binhluan);
    if (result.isAppropriate === false) {
      toast.error(`Đánh giá không phù hợp: ${result.message}`);
      setIsLoading(false);
      return;
    }
        ...
        <Form {...form}>
          ...
          >
                ...
                >
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-600">
                      Kéo thả ảnh hoặc click để tìm kiếm
                    </p>
                    <input
                      type="file"
            ...
            <div className="flex gap-3 pt-4">
              ...
              >
                {isPending || isLoading ? 'Đang gửi...' : 'Gửi đánh giá'}
              </Button>
            </div>
          </form>
        </Form>
